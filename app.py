...
Policy Q&A System — Makerere University
"""
...
def text_to_speech(text):
    try:
        ...
        engine.runAndWait()
        engine.stop()
        return tmp.name if os.path.getsize(tmp.name) > 1000 else None
...
def transcribe_audio(audio_bytes):
    ...
    try:
        ...
        cmd = ["ffmpeg", "-y", "-i", raw.name, "-ar", "16000", "-ac", "1", "-acodec", "pcm_s16le", wav_path]
        r = subprocess.run(cmd, capture_output=True)
        os.unlink(raw.name)

        wav_size = os.path.getsize(wav_path) if os.path.exists(wav_path) else 0
        debug_msgs.append(f"ffmpeg exit={r.returncode}, wav={wav_size} bytes")

        if wav_size < 5000:
            debug_msgs.append("Conversion failed — wav too small")
            return "", debug_msgs

        import whisper as whisper_lib
        audio_np = whisper_lib.load_audio(wav_path)
        audio_np = whisper_lib.pad_or_trim(audio_np)
        debug_msgs.append(f"Audio max amplitude: {float(np.abs(audio_np).max()):.4f}")
        os.unlink(wav_path)

        model = load_whisper()
        mel = whisper_lib.log_mel_spectrogram(audio_np).to(model.device)
        options = whisper_lib.DecodingOptions(language="en", fp16=False)
        ...
        if not stripped or len(text.split()) < 2:
            debug_msgs.append("Rejected — too short or silence")
            return "", debug_msgs
...
GREETING_RESPONSE = (
    ...
    "I can help you understand university policies on safeguarding, "
...
def clean_text(text):
    ocr_fixes = {
        ...
        "intothe": "into the", "Directorateof": "Directorate of",
        "Mainstreamingor": "Mainstreaming or", "Mainstreamingto": "Mainstreaming to",
        "GenderMainstreaming": "Gender Mainstreaming", "Deanof": "Dean of",
        "complaintsof": "complaints of", "complaintof": "complaint of",
        "complaintshould": "complaint should", "evidenceof": "evidence of",
        "casesof": "cases of", "formsof": "forms of", "typesof": "types of",
        "reportof": "report of", "victimsto": "victims to", "victimof": "victim of",
        "personin": "person in", "personwho": "person who", "personwith": "person with",
        "actioncan": "action can", "actionmust": "action must", "stepsto": "steps to",
        "ableto": "able to", "failsto": "fails to", "needto": "need to",
        "hasto": "has to", "wantto": "want to", "doingany": "doing any",
        "takingany": "taking any", "subjectto": "subject to", "accessto": "access to",
        "rightto": "right to", "entitledto": "entitled to",
        "specialcir cumstances": "special circumstances",
        "specialcircumstances": "special circumstances", "cir cumstances": "circumstances",
        "har assment": "harassment", "haras sment": "harassment",
        "dis ability": "disability", "com plaint": "complaint",
        "investiga tion": "investigation", "com mittee": "committee",
        "har- assment": "harassment", "dis- ability": "disability",
        "re- port": "report", "com- plaint": "complaint",
    }
...
def polish_with_groq(question, raw_policy_text):
    if not GROQ_API_KEY:
        return None
    system_prompt = (
        "You are a university safeguarding assistant for Makerere University students. "
        "Explain university policies clearly and simply. Rules:\n"
        "1. Fix OCR errors like merged words (e.g. 'mustbe' -> 'must be').\n"
        "2. Use a numbered list for steps or procedures.\n"
        "3. Use bullet points starting with • for general information.\n"
        "4. Use ONLY the policy text provided.\n"
        "5. Remove repeated sentences and keep it concise.\n"
        "6. End with: 'For more help, contact the Directorate of Gender Mainstreaming "
        "at gendermainstreaming@mak.ac.ug or call +256 (0)414 532 631.'"
    )
    user_prompt = (
...
def clean_sentence(s):
    s = re.sub(r"\b([a-zA-Z]{2,})\s([a-z]{1,3})\b",
               lambda m: m.group(1) + m.group(2) if m.group(2) not in STOP_WORDS else m.group(0), s)
...
def generate_answer(question, retrieved):
    ...
    return polished if polished else format_chunks_as_bullets(retrieved, query=question)
...
def load_everything():
    ...
    if not (csv_ok and npy_ok):
        raise RuntimeError("Could not load chunks from GitHub.")
...
with st.sidebar:
    ...
    nc = st.slider("contrast", 50, 150, st.session_state.contrast, 5, label_visibility="collapsed", key="contrast_slider")
    ...
    nf = st.slider("font", 12, 22, st.session_state.font_size, 1, label_visibility="collapsed", key="font_slider")
...
<div style="font-size:0.72rem;font-weight:700;color:#ff6b6b;margin-bottom:8px;">🚨 NEED IMMEDIATE HELP?</div>
...
<span style="color:#8b949e;">Mon–Fri · 8 AM – 5 PM · Frank Kalimuzo Building</span>
</div>
</div>
""", unsafe_allow_html=True)
...
if active_conv and not active_conv["messages"]:
    ...
    <div class="welcome-card">
      ...
      disability rights, sexual harassment procedures, and student protections.</p>
    </div>
    """, unsafe_allow_html=True)
...
with st.expander("🎙️ Speak your question instead of typing", expanded=False):
    ...
    if audio_input is not None:
        ...
        if transcribed:
            ...
            st.warning("Could not transcribe. You can type what you said below:")
    ...
    with st.chat_message("assistant", avatar="🛡️"):
        ...
        try:
            ...
            if audio_path:
                ...
                st.session_state["last_audio_err"] = "TTS returned no audio"
        except Exception as e:
            st.session_state["last_audio"] = None
        ...
        full_content = answer
        if sources:
            full_content += "<br>" + "".join(f'<span class="src-tag">📄 {nice_source_name(s)}</span>' for s in sources)
...
if st.session_state.get("last_audio"):
    ...
    st.caption("🔊 Tap play to hear this answer")
    st.session_state["last_audio"] = None
elif st.session_state.get("last_audio_err"):
    st.caption(f"🔇 Audio unavailable: {st.session_state['last_audio_err']}")
