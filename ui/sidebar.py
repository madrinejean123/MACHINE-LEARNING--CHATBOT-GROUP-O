...
def render_sidebar(theme: dict):
    ...
    # DEBUG: prove the function is being called
    st.sidebar.markdown("### 🔍 SIDEBAR DEBUG")
    st.sidebar.write("render_sidebar(theme) was called")
    st.sidebar.write(f"Theme loaded: BORDER={BORDER}, TEXT={TEXT}, SUBTEXT={SUBTEXT}")

    try:
        with st.sidebar:

            # Extra debug inside sidebar container
            st.markdown("✅ Sidebar container is active", unsafe_allow_html=False)
    ...
    """, unsafe_allow_html=True)
            ...
            # DEBUG: prove we reached the end
            st.markdown("### ✅ SIDEBAR DEBUG END", unsafe_allow_html=False)
            st.write("Sidebar finished rendering")

    except Exception as e:
        st.sidebar.error(f"Sidebar error: {type(e).__name__}: {e}")
        st.sidebar.write("The sidebar function started, but crashed before finishing.")